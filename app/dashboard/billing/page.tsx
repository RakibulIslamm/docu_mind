...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  no_subscription: "Couldn't find an active subscription to cancel.",
...
export default async function BillingPage({
  ...
  // Try the extended select first. If migration 0003_billing_state.sql hasn't
  // been applied yet, the three new columns don't exist and Supabase returns
  // an error (no data). Fall back to the basic columns so the page still
  // renders correctly off `plan` + live Stripe state — the cached subscription
  // columns are an optimization, not a requirement.
  let profile: {
    plan: string | null
  ...
  if (extended.error) {
    console.warn(
      "[billing] extended profile select failed; falling back to basic columns. Apply supabase/migrations/0003_billing_state.sql.",
    ...
    if (basic.data) {
      profile = {
        plan: basic.data.plan ?? null,
        stripe_customer_id: basic.data.stripe_customer_id ?? null,
  ...
  if (stripeCfg.configured && profile?.stripe_customer_id) {
    ...
    if (customerRes.status === "fulfilled" && !customerRes.value.deleted) {
      ...
      console.warn("[stripe] could not retrieve customer:", customerRes.reason)
    }
    ...
    if (invoicesRes.status === "fulfilled") {
      ...
      console.warn("[stripe] could not list invoices:", invoicesRes.reason)
    }
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-3xl space-y-4 px-4 py-8 sm:px-6 lg:px-10">
        ...
        {/* Current plan summary with manage / cancel / resume actions. */}
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {plan === "pro"
                ? "Unlimited documents, multi-document chats, and priority model latency."
                : "Free tier — 3 documents, 50 questions per month."}
            </CardDescription>
          </CardHeader>
  ...
  if (subscriptionStatus === "past_due") {
    return (
      <div className="flex items-start gap-2 border border-destructive/40 px-3 py-2 text-xs text-destructive">
        ...
        <span>
          Payment failed. Update your card from{" "}
          <strong>Manage subscription</strong> before access drops to Free.
        </span>
      </div>
    )
  }
 ...
 * Stripe's documented test card numbers so visitors can drive the Pro upgrade
 * flow end-to-end without entering real card details. Auto-disappears the
 * moment the operator swaps to a live key.
 */
    ...
    <Card className="border-amber-500/40 bg-amber-500/5">
      <CardHeader>
        ...
        <CardDescription>
          ...
          rejected — pick one of the numbers below to drive the upgrade flow.
        ...
        <ul className="divide-y divide-border border-y border-border">
          {cards.map((c) => (
            ...
            >
              ...
              <Badge
                variant="outline"
                className={cn("uppercase", toneClass(c.tone))}
              >
                {c.label}
              </Badge>
...
function formatDate(d: Date): string {
  ...
  // invoice emails, and receipts. Otherwise a 19:48 UTC timestamp can
  // look like "Jun 10" to a user in UTC+6 even though Stripe says "Jun 9".
